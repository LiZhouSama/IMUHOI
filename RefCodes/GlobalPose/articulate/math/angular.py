...
def angle_between(rot1: torch.Tensor, rot2: torch.Tensor, rep=RotationRepresentation.ROTATION_MATRIX):
    ...
    offsets = rot1.bmm(rot2.transpose(1, 2))   # 替换原来的 rot1.transpose(1, 2).bmm(rot2)
...
def rotation_matrix_to_axis_angle(r: torch.Tensor):
    ...
    # return result
    """
    Pure-Torch, stable conversion. Matches PyTorch3D's matrix_to_axis_angle behavior.
    Input:  r [..., 3, 3]
    Output: aa [..., 3]  (axis * angle), angle in [0, pi]
    """
    R = r.view(-1, 3, 3)
    # trace -> cos(theta)
    trace = R[:, 0, 0] + R[:, 1, 1] + R[:, 2, 2]
    cos_theta = (trace - 1.0) * 0.5
    # clamp to avoid NaNs from numerical drift
    cos_theta = cos_theta.clamp(min=-1.0 + 1e-6, max=1.0 - 1e-6)
    ...
    wy = R[:, 0, 2] - R[:, 2, 0]
    wz = R[:, 1, 0] - R[:, 0, 1]
    vee = torch.stack([wx, wy, wz], dim=1)

    # general case (not too small, not near pi): w = theta/(2*sin(theta)) * vee
    sin_theta = torch.sqrt(1.0 - cos_theta * cos_theta)  # = |sin(theta)|
    scale = (theta / (2.0 * (sin_theta + 1e-8))).unsqueeze(1)
    aa = scale * vee

    # small-angle branch: theta ~ 0, use first-order approx w ≈ 0.5 * vee
    small = theta < 1e-4
    if small.any():
        aa[small] = 0.5 * vee[small]

    # near-pi branch: theta ~ pi, use robust axis extraction from diagonals
    near_pi = theta > (np.pi - 1e-3)
    if near_pi.any():
        Rp = R[near_pi]
        aa_pi = torch.zeros_like(vee[near_pi])

        # choose the largest diagonal to improve numerical stability
        r00, r11, r22 = Rp[:, 0, 0], Rp[:, 1, 1], Rp[:, 2, 2]
        case0 = (r00 >= r11) & (r00 >= r22)
        case1 = (r11 > r00) & (r11 >= r22)
        case2 = (~case0) & (~case1)
        ...
        if case0.any():
            Rc = Rp[case0]
            x = torch.sqrt(torch.clamp(1.0 + Rc[:, 0, 0] - Rc[:, 1, 1] - Rc[:, 2, 2], min=0.0)) / 2.0
            y = (Rc[:, 0, 1] + Rc[:, 1, 0]) / (4.0 * (x + eps))
            z = (Rc[:, 0, 2] + Rc[:, 2, 0]) / (4.0 * (x + eps))
            axis = torch.stack([x, y, z], dim=1)
            axis = axis / (axis.norm(dim=1, keepdim=True) + eps)
            aa_pi[case0] = axis * theta[near_pi][case0].unsqueeze(1)

        # case1: y largest
        if case1.any():
            Rc = Rp[case1]
            y = torch.sqrt(torch.clamp(1.0 - Rc[:, 0, 0] + Rc[:, 1, 1] - Rc[:, 2, 2], min=0.0)) / 2.0
            x = (Rc[:, 0, 1] + Rc[:, 1, 0]) / (4.0 * (y + eps))
            z = (Rc[:, 1, 2] + Rc[:, 2, 1]) / (4.0 * (y + eps))
            axis = torch.stack([x, y, z], dim=1)
            axis = axis / (axis.norm(dim=1, keepdim=True) + eps)
            aa_pi[case1] = axis * theta[near_pi][case1].unsqueeze(1)

        # case2: z largest
        if case2.any():
            Rc = Rp[case2]
            z = torch.sqrt(torch.clamp(1.0 - Rc[:, 0, 0] - Rc[:, 1, 1] + Rc[:, 2, 2], min=0.0)) / 2.0
            x = (Rc[:, 0, 2] + Rc[:, 2, 0]) / (4.0 * (z + eps))
            y = (Rc[:, 1, 2] + Rc[:, 2, 1]) / (4.0 * (z + eps))
            axis = torch.stack([x, y, z], dim=1)
            axis = axis / (axis.norm(dim=1, keepdim=True) + eps)
            aa_pi[case2] = axis * theta[near_pi][case2].unsqueeze(1)
...
def r6d_to_rotation_matrix(r6d: torch.Tensor):
    ...
    **Warning:** The two 3D vectors of any 6D vector must be linearly independent.
    ...
    column0 = F.normalize(r6d[:, 0:3], dim=-1, eps=1e-6)
    proj     = (column0 * r6d[:, 3:6]).sum(dim=1, keepdim=True)
    column1 = F.normalize(r6d[:, 3:6] - proj * column0, dim=-1, eps=1e-6)
    column2 = column0.cross(column1, dim=1)
    # r = torch.stack((column0, column1, column2), dim=-1)
    r = torch.stack((column0, column1, column2), dim=-2)
    r[torch.isnan(r)] = 0
    return r
...
def rotation_matrix_to_r6d(r: torch.Tensor):
    ...
    """
    R = r.view(-1, 3, 3)
    return R[:, :2, :].contiguous().view(-1, 6)   # 与 p3d 一致
