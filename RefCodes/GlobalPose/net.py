...
# robust import for carticulate: prefer installed package; fallback to carticulate_raw.utils
try:
    import carticulate as cart
    try:
        # if top-level helpers are missing, try injecting from scripts.utils
        if not hasattr(cart, 'get_dynamic_model') or not hasattr(cart, 'get_kinematic_model'):
            from carticulate.scripts import utils as _cart_utils  # type: ignore
            if not hasattr(cart, 'get_dynamic_model'):
                cart.get_dynamic_model = _cart_utils.get_dynamic_model  # type: ignore
            if not hasattr(cart, 'get_kinematic_model'):
                cart.get_kinematic_model = _cart_utils.get_kinematic_model  # type: ignore
    except Exception:
        pass
except Exception:
    # windows prebuilt fallback
    from carticulate import utils as cart  # type: ignore
...
class GPNet(torch.nn.Module):
    ...
    kp_pose = 900      # kp in pose PD controller (30 Hz tuned)
    kd_pose = 30       # kd in pose PD controller (30 Hz tuned)
    kp_tran = 900      # kp in tran PD controller (30 Hz tuned)
    kd_tran = 30       # kd in tran PD controller (30 Hz tuned)
    alpha_pd = 0.0     # relaxation in stable PD controller
    floor_y = 0.0      # floor height (OMOMO data aligned at ground=0)
    beta_velocity = 1
    beta_cjoint = 1
    beta_extforce = 0.4
    beta_torque = 1e-3 / 80
    ...
    def __init__(self, pretrained_path: str = 'data/weights.pt'):
        ...
        if pretrained_path:
            ...
            if weight_path.exists():
                ...
                print(f'[GPNet] Warning: pretrained weights not found at {weight_path}, initializing randomly.')
    ...
    def _explain_residual_force(self, contact, contact_Jacobian, contact_position, residual_force):
        if np.any(contact):
            ...
            P = csc_matrix(JTB.T @ JTB + self.beta_extforce * np.eye(JTB.shape[1]))
            q = -JTB.T @ residual_force
            lb_vec = np.concatenate(lb)
            G = -eye(lb_vec.size, format="csc")
            h = -lb_vec
            force = solve_qp(P, q, G=G, h=h, solver='osqp')
    ...
    def forward_frame(self, a, w, R):
        ...
        if not self.is_init:
            ...
            qddot = lsqr(csc_matrix(A), b)[0]
            # 数值稳健性保护：去除 NaN/Inf 并裁剪过大加速度，避免后续 SO3::exp 爆炸
            qddot = np.nan_to_num(qddot, nan=0.0, posinf=0.0, neginf=0.0)
            qddot_max = 500.0
            qddot = np.clip(qddot, -qddot_max, qddot_max)
            ...
            qddot = np.clip(qddot, -qddot_max, qddot_max)
            # 按 C++ 接口期望形状/类型传递
            self.physics_model.update_state(qddot.astype(np.float32).reshape(-1, 1), self.dt)
