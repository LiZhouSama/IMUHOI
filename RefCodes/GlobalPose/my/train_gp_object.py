...
使用真实的 root translation，不再进行网络估计。
"""
...
from my.losses_object import GPNetWithObjectLoss
from my.models.gpnet_object import GPNetWithObject


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description='Train GPNet with object tracking on OMOMO dataset (GT root translation).'
    )
    parser.add_argument(
    ...
    parser.add_argument(
        '--save_dir', type=str, default='my/checkpoints/gp_object_BEHAVE'
    )
    parser.add_argument(
...
def _collect_root_meta(
    base_dataset,
    indices: Optional[List[int]]
) -> Optional[List[Optional[Dict[str, torch.Tensor]]]]:
    """
    Gather ground-truth root translation/rotation for injection during forward.
    """
    if indices is None or not isinstance(base_dataset, OMOMOGlobalPoseDataset):
        return None
    meta_list: List[Optional[Dict[str, torch.Tensor]]] = []
    for idx in indices:
        meta = base_dataset.get_sequence_meta(idx)
        if not meta:
            meta_list.append(None)
            continue
        meta_list.append({
            'tran': meta.get('tran'),
            'root_rot': meta.get('root_rot'),
        })
    return meta_list
...
def build_object_inputs(
    ...
    for idx, human_pred in zip(data_indices, human_preds):
        ...
        # Create init vector (RNNWithInit requires the first-frame target as init state)
...
def run_epoch(
    ...
    for step, batch in enumerate(loader, start=1):
        ...
        root_meta = _collect_root_meta(base_dataset, batch_indices)

        # Human forward pass (root velocity is replaced with ground truth)
        human_preds, _ = model(data, x_object=None, fast=True, root_meta=root_meta)
        
        # Object forward pass (if available)
        obj_preds, obj_targets = None, None
        if train_object and batch_indices is not None and isinstance(base_dataset, OMOMOGlobalPoseDataset):
