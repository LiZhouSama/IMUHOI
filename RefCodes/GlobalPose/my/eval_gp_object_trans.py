"""
Evaluation script for GPNet with object tracking on OMOMO sequences.
Outputs MPJPE/MPJRE/root/object errors matching DynaIP evaluation metrics.
"""
...
from my.models.gpnet_object_trans import GPNetWithObject  # noqa: E402
...
DEFAULT_DATASET_CONFIG: Dict[str, Dict[str, Path]] = {
    "processed_seg_data_IMHD": {
        ...
        "checkpoint": SCRIPT_DIR / "checkpoints" / "gp_object_trans_IMHD" / "best.pt",
    },
    "processed_seg_data_BEHAVE": {
        "data_dir": PROJECT_ROOT / "process" / "processed_seg_data_BEHAVE" / "test",
        "checkpoint": SCRIPT_DIR / "checkpoints" / "gp_object_trans_BEHAVE" / "best.pt",
    },
    "processed_split_data_OMOMO": {
        "data_dir": PROJECT_ROOT / "process" / "processed_split_data_OMOMO" / "test",
        "checkpoint": SCRIPT_DIR / "checkpoints" / "gp_object_trans_OMOMO" / "best.pt",
    },
}
...
def evaluate_model(
    ...
    for idx in range(total):
        try:
            ...
            human_pred_list, _ = model([(seq_input, init_vec)], x_object=None, fast=True)
            ...
            tran_pred = torch.stack(tran_frames, dim=0).to(device=device, dtype=dtype)
            root_offset = tran_gt[0]
