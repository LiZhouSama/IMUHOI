...
def compare_realimu(data, dataset_name='', save_results_dir='data/temp/results', evaluate_pose=True, evaluate_tran=True):
    ...
    if evaluate_tran:
        ...
        for net_name in tran_errors.keys():
            keys_sorted = list(tran_errors[net_name].keys())
            means, stds = [], []
            for e in tran_errors[net_name].values():
                if len(e) > 0:
                    means.append(torch.stack([_.mean() for _ in e]).mean().item())
                    stds.append(torch.stack([_.std() for _ in e]).mean().item())
                else:
                    means.append(np.nan)
                    stds.append(np.nan)
            x = np.array([0] + keys_sorted)
            y = np.array([0] + means)
            y_std = np.array([0] + stds)
        ...
        for k, v in tran_errors.items():
            if len(v[7]) > 0:
                drift_pct = torch.stack([_.mean() for _ in v[7]]).mean().item() / 7 * 100
                lines.append('%s  %.2f%%' % (k, drift_pct))
            else:
