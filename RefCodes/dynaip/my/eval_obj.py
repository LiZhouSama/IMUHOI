...
def evaluate_obj_model(
    ...
    if model.body_model is None:
        raise RuntimeError(
            ...
            "Instantiate PoserWithObject with a valid SMPL body model path."
        )
    ...
    for batch_idx, batch in enumerate(data_loader):
        ...
        # )
        B, T = batch["pose"].shape[0], batch["pose"].shape[1]
        BT = B * T
        pose_6d = batch["pose"].reshape(B*T, -1, 6)
        pose_aa = transforms.rotation_6d_to_matrix(pose_6d)
        pose_aa = transforms.matrix_to_axis_angle(pose_aa)
        body_out = model.body_model(
            pose_body=pose_aa[:, 1:22].reshape(BT, 63),
            root_orient=pose_aa[:, 0].reshape(BT, 3),
            trans=torch.zeros(BT, 3, device=device, dtype=batch["pose"].dtype),
        )
        joints_gt = body_out.Jtr[:, :24, :].to(device).view(B, T, 24, 3)

        # MPJPE: 归一化到各自根关节坐标系（去除全局位移影响）
        mpjpe_val = torch.linalg.norm(joints_pred_rel - joints_gt, dim=-1).mean().item() * 100.0
        ...
        pred_body_6d = transforms.matrix_to_rotation_6d(local_pose_pred[:, :, :22, :, :].reshape(-1, 3, 3)).reshape(B, T, -1, 6)
        # gt_body_6d = transforms.matrix_to_rotation_6d(gt_local.reshape(-1, 3, 3)).reshape(B, T,-1, 6)
        rot_error_ = torch.mean(torch.absolute(pose_6d-pred_body_6d)) * 57.2958
        ...
        if evaluate_objects and "obj_pos" in batch:
            ...
            hoi_err = _compute_hoi_error(
                pred_obj_pos, gt_obj_pos, joints_pred_rel, joints_gt, batch
            )
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--checkpoint",
        type=str,
        default="checkpoints/dynaip_obj_behave/best_val.pth",
