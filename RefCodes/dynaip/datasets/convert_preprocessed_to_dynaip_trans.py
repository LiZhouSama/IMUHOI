...
def compute_joint_velocity_and_position_like_process_xsens(rotation_global: torch.Tensor,
    ...
    Match Ref Codes/dynaip/datasets/process.py::process_xsens
      - remove horizontal root x/y motion
...
def convert_split(source_dir: str, target_root: str, dataset_name: str, fps: float = 60.0):
    ...
    for pt in pt_files:
        ...
        # Extract foot contact labels from preprocessed data
        lfoot_contact = d.get('lfoot_contact', torch.zeros(rot.shape[0])).float()
        rfoot_contact = d.get('rfoot_contact', torch.zeros(rot.shape[0])).float()
        
        # Compute root velocity in world frame for training
        root_velocity_world = torch.zeros(rot.shape[0], 3)
        root_velocity_world[1:] = (pos[1:, 0, :] - pos[:-1, 0, :]) * fps
        
        out = {
            'joint': {
                'orientation': orient16.reshape(rot.shape[0], -1)[6:-6],  # [T, 16*6]
                'velocity': vel_root[6:-6],     # m/s
                'position': pos_root[6:-6],
                'root_velocity_world': root_velocity_world[6:-6],  # 新增：根关节世界坐标系速度, m/s
            },
            'imu': {
                'imu': imu[6:-6],
            },
            'contact': {
                'lfoot': lfoot_contact[6:-6],  # 新增：左脚接触标签
                'rfoot': rfoot_contact[6:-6],  # 新增：右脚接触标签
            }
        }
