"""
Evaluation utilities for the TransPose-with-object model.
"""
...
from my.model_trans_obj import TransPoseWithObject


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Evaluate TransPose-with-object model.")
    parser.add_argument('--checkpoint', type=str, default='checkpoints/transpose_trans_object_omomo/best_val.pth',
...
def _compute_metrics(pred: Dict[str, torch.Tensor], gt: Dict[str, torch.Tensor], 
    ...
    pred_pose_6d = pred['reduced_pose'].reshape(B * T, -1)  # [B*T, n_reduced*6]
    root_rot_flat = root_rotation.reshape(B * T, 3, 3)
    # # 转换为局部旋转矩阵 [B*T, 24, 3, 3]
    ...
    # 3. Root Trans Error: 根位移误差
    if 'root_position_world' in pred and 'root_position_world' in gt:
        root_diff = pred['root_position_world'] - gt['root_position_world']
        root_trans_err = torch.linalg.norm(root_diff, dim=-1).mean().item() * 100.0  # cm
        metrics['root_trans_err'] = root_trans_err
    
    # 4. Obj Trans Error: 物体位移误差
    ...
    if 'obj_position' in pred and 'obj_position' in gt and 'full_pos' in pred and 'full_pos' in gt:
        ...
        # 添加根位移得到世界坐标系的手腕位置
        pred_root = pred.get('root_position_world', torch.zeros_like(pred['obj_position']))
        gt_root = gt.get('root_position_world', torch.zeros_like(gt['obj_position']))
        
        pred_wrists_world = pred_wrists + pred_root.unsqueeze(2)
        gt_wrists_world = gt_wrists + gt_root.unsqueeze(2)
        
        # 计算相对关系误差
        # pred_obj_pos: [B, T, 3] -> [B, T, 1, 3]
        rel_pred = pred['obj_position'].unsqueeze(2) - pred_wrists_world
        rel_gt = gt['obj_position'].unsqueeze(2) - gt_wrists_world
    ...
    # 6. Jitter: 关节加速度抖动（mm/frame^2）
    if 'full_pos' in pred and 'root_position_world' in pred:
        # 使用世界坐标系的关节位置（包含根位移）
        pred_joints_world = joints_pred + pred['root_position_world'].unsqueeze(2)
        pred_joints_eval = pred_joints_world[:, :, :22, :]  # [B, T, 22, 3]
...
def evaluate() -> None:
    ...
    for batch_idx, batch in enumerate(iterator):
        ...
        gt = {
            ...
            'root_position_world': batch['root_position_world'],
    ...
    print(f"Root Trans Error (cm):  {averaged.get('root_trans_err', float('nan')):.4f}")
