...
from my_simple.dataset_obj import MotionDatasetWithObjectAndTrans, collate_fn_with_object_and_trans
from my_simple.model_obj import PoserWithObject
from my_simple.loss_obj import loss_p_obj
...
def train_obj():
    ...
    parser.add_argument('--save_dir', type=str, default=os.path.join('./checkpoints/', 'dip_obj_omomo'),
    ...
    parser.add_argument('--datasets_train', nargs='+', default=['processed_split_data_OMOMO'], help='Names of dataset subsets inside data_root/<name>/train.')
    parser.add_argument('--datasets_test', nargs='+', default=['processed_split_data_OMOMO'], help='Names of dataset subsets inside data_root/<name>/test.')
    ...
    def evaluate_validation():
        ...
        with torch.no_grad():
            for batch in val_loader:
                ...
                p_pred, obj_v_pred = model(
                    batch['imu'], batch['obj_imu']
                )
    ...
    for epoch in range(args.num_epochs):
        ...
        print(f"Epoch {epoch + 1:03d} | Train Loss: {train_loss:.4f} | "
              f"P: {loss_details['loss_p']:.4f} | Obj: {loss_details['loss_obj']:.4f} | "
