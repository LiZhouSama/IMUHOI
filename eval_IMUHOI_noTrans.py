...
OUTPUT_IMUHOI_NOTRANS_ROOT = PROJECT_ROOT / "outputs" / "IMUHOI_noTrans"

DEFAULT_DATASET_CONFIG = {
    "processed_seg_data_BEHAVE": {
        "data_dir": PROCESS_ROOT / "processed_seg_data_BEHAVE" / "test",
        "model_path": OUTPUT_IMUHOI_NOTRANS_ROOT / "transpose_noTrans_behave" / "stage_best_joint_training.pt",
        "modules": {
            "velocity_contact": OUTPUT_IMUHOI_NOTRANS_ROOT
            / "transpose_noTrans_behave"
            / "modules"
            / "velocity_contact_best.pt",
            "human_pose": OUTPUT_IMUHOI_NOTRANS_ROOT
            / "transpose_noTrans_behave"
            / "modules"
            / "human_pose_best.pt",
            "object_trans": OUTPUT_IMUHOI_NOTRANS_ROOT
            / "transpose_noTrans_behave"
            / "modules"
    ...
    "processed_seg_data_IMHD": {
        ...
        "model_path": OUTPUT_IMUHOI_NOTRANS_ROOT / "transpose_noTrans_imhd" / "stage_best_joint_training.pt",
        "modules": {
            "velocity_contact": OUTPUT_IMUHOI_NOTRANS_ROOT
            / "transpose_noTrans_imhd"
            / "modules"
            / "velocity_contact_best.pt",
            "human_pose": OUTPUT_IMUHOI_NOTRANS_ROOT
            / "transpose_noTrans_imhd"
    ...
    "processed_split_data_OMOMO": {
        ...
        "model_path": OUTPUT_IMUHOI_NOTRANS_ROOT / "transpose_noTrans_omomo" / "stage_best_joint_training.pt",
        "modules": {
            "velocity_contact": OUTPUT_IMUHOI_NOTRANS_ROOT
            / "transpose_noTrans_omomo"
            / "modules"
            / "velocity_contact_best.pt",
            "human_pose": OUTPUT_IMUHOI_NOTRANS_ROOT
            / "transpose_noTrans_omomo"
        ...
        for batch_idx, batch in enumerate(data_loader):
            ...
            for sample_idx in range(batch_size):
                ...
                if pred_hand_vel is not None and T >= 2:
                    pred_hand_vel_seq = pred_hand_vel[sample_idx]  # [T, 2, 3]
                    
                    # Compute GT hand velocity from GT joints (using finite differences)
                    wrist_l_idx, wrist_r_idx = 20, 21
                    gt_lhand_pos = gt_joints_all[:, wrist_l_idx, :]  # [T, 3]
                    gt_rhand_pos = gt_joints_all[:, wrist_r_idx, :]  # [T, 3]
                    
                    dt = 1.0 / float(FRAME_RATE)
                    # Compute velocity using forward differences
                    gt_lhand_vel = (gt_lhand_pos[1:] - gt_lhand_pos[:-1]) / dt  # [T-1, 3]
                    gt_rhand_vel = (gt_rhand_pos[1:] - gt_rhand_pos[:-1]) / dt  # [T-1, 3]
                    
                    # Use the predicted velocity at corresponding frames (exclude last frame)
                    pred_lhand_vel = pred_hand_vel_seq[:-1, 0, :]  # [T-1, 3]
                    pred_rhand_vel = pred_hand_vel_seq[:-1, 1, :]  # [T-1, 3]
                    
                    # Compute L2 error in m/s
                    ...
                    avg_vel_err = (lhand_vel_err + rhand_vel_err) / 2.0
                    
                    metrics["hand_vel_err_lhand"].append(lhand_vel_err)
                    metrics["hand_vel_err_rhand"].append(rhand_vel_err)
                    metrics["hand_vel_err_avg"].append(avg_vel_err)
                else:
                    metrics["hand_vel_err_lhand"].append(float("nan"))
                    metrics["hand_vel_err_rhand"].append(float("nan"))
                    metrics["hand_vel_err_avg"].append(float("nan"))
...
def main():
    ...
    parser.add_argument("--config", type=str, default="configs/IMUHOI_train_noTrans.yaml", help="Path to config file")
    parser.add_argument(
        "--dataset",
        type=str,
    ...
    parser.add_argument("--smpl_model_path", type=str, default="datasets/smpl_models/smplh/male/model.npz", help="Override SMPL model path")
    parser.add_argument(
        "--test_data_dir",
        type=str,
        default=None,
    ...
    for dataset_name, dataset_cfg in dataset_runs:
        ...
        print("\n--- Hand Velocity Errors (m/s) ---")
        print(f"Left Hand:                      {_fmt('hand_vel_err_lhand')}")
        print(f"Right Hand:                     {_fmt('hand_vel_err_rhand')}")
        print(f"Average:                        {_fmt('hand_vel_err_avg')}")
