...
from models.TransPose_net_0706 import TransPoseNet, joint_set
...
def do_train_imu_TransPose(cfg, train_loader, test_loader=None, trial=None, model=None, optimizer=None):
    ...
    for epoch in range(max_epoch):
        ...
        for batch in train_iter:
            ...
            foot_contact_prob_watch = foot_contact_prob.clone().detach().cpu().numpy()
            foot_contact_gt_watch = foot_contact_gt.clone().detach().cpu().numpy()
...
def load_transpose_model(cfg, checkpoint_path):
    ...
    # 确保CUDA环境正确初始化
    if device.type == 'cuda':
        # 创建一个小的tensor来初始化cuDNN
        torch.cuda.empty_cache()
        dummy_tensor = torch.randn(1, 1, device=device)
        dummy_tensor = dummy_tensor + 1  # 简单操作确保cuDNN初始化
        del dummy_tensor
        torch.cuda.empty_cache()
