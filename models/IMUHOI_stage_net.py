...
class VelocityContactModule(nn.Module):
    ...
    def forward(
        ...
        return {
            ...
            "pred_hand_glb_vel_flat": hand_vel.view(batch_size, seq_len, -1),
...
class HumanPoseModule(nn.Module):
    ...
    def _compute_torso_velocity_batched(self, joints_pos: torch.Tensor):
        if joints_pos is None:
            return None
        assert joints_pos.dim() == 4 and joints_pos.shape[2] >= 16, f"joints_pos shape invalid: {joints_pos.shape}"
    ...
    def forward(self, human_imu: torch.Tensor, v_init: torch.Tensor, p_init: torch.Tensor, trans_init: torch.Tensor):
        if human_imu.dim() != 4:
            raise ValueError(f"human_imu must be [B, T, num_imu, imu_dim], got {human_imu.shape}")
        batch_size, seq_len, _, _ = human_imu.shape
        device = human_imu.device
    ...
    def forward(self, data_dict, use_object_data=None, compute_fk=False):
        ...
        if self.velocity_contact_module is not None:
            ...
            vc_out = {
                ...
                "pred_hand_glb_vel_flat": torch.zeros(batch_size, seq_len, 6, device=human_imu.device),
        ...
        if self.human_pose_module is not None:
            ...
            hp_out = {
                ...
                "root_trans_pred": torch.zeros(batch_size, seq_len, 3, device=human_imu.device),
