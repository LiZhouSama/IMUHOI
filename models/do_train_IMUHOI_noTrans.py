...
LOSS_KEYS_BY_MODULE = {
    ...
    'human_pose': {'vel_root', 'pose_reduced'},
    'object_trans': {
        'obj_trans',
        'lhand_obj_direction',
        'rhand_obj_direction',
...
def _compute_loss_terms(pred_dict, batch, stage_info, device, model=None):
    ...
    trans_gt = batch.get('trans').to(device)
    root_vel_gt = batch.get('root_vel').to(device)
    root_vel_local_gt = root_ori_gt.transpose(-1, -2).matmul(root_vel_gt.unsqueeze(-1)).squeeze(-1)

    sensor_vel_root_gt = batch.get('sensor_vel_root').to(device)
    if sensor_vel_root_gt.dim() == 3:
        sensor_vel_root_gt = sensor_vel_root_gt.unsqueeze(0).expand(bs, -1, -1, -1)

    sensor_vel_glb_gt = batch.get('sensor_vel_glb').to(device)
    if sensor_vel_glb_gt.dim() == 3:
        sensor_vel_glb_gt = sensor_vel_glb_gt.unsqueeze(0).expand(bs, -1, -1, -1)

    ori_root_reduced_gt = batch.get('ori_root_reduced').to(device)
    position_global_gt = batch.get('position_global').to(device)

    obj_vel_gt = batch.get('obj_vel').to(device)
    obj_imu_gt = batch.get('obj_imu').to(device)
    obj_trans_gt = batch.get('obj_trans').to(device)
    lhand_dir_gt = batch.get('lhand_obj_direction').to(device)
    rhand_dir_gt = batch.get('rhand_obj_direction').to(device)
    lhand_contact_gt = batch.get('lhand_contact').to(device).bool()
    rhand_contact_gt = batch.get('rhand_contact').to(device).bool()
    obj_contact_gt = batch.get('obj_contact').to(device).bool()
...
def compute_stage_specific_test_loss(pred_dict, batch, stage_info, cfg, device, model=None):
    active_modules = set(stage_info['active_modules'])
    loss_dict = _compute_loss_terms(pred_dict, batch, stage_info, device, model)
    total_loss, _ = _apply_loss_weights(loss_dict, cfg, active_modules, device)
