...
from models.do_train_IMUHOI_noTrans import do_train_imu_TransPose, compute_stage_schedule
...
def get_args():
    ...
    parser.add_argument('--cfg', type=str, default='configs/IMUHOI_train_noTrans.yaml', help='配置文件路径')
...
def main():
    ...
    run_name = f"transpose_noTrans_{time_stamp}"
    ...
    # 打印训练配置
    print("=" * 50)
    print(f"模型类型: TransPose (noTrans)")
    ...
    print(f"训练完成！TransPose (noTrans) 模型已保存到 {cfg.save_dir}")
